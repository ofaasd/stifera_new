<?php

namespace App\Http\Controllers;

use App\Http\Controllers\Controller;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\DB;
use PhpOffice\PhpSpreadsheet\IOFactory;

class PegawaiNilaiController extends Controller
{
    /**
     * Resolve pegawai_biodata.id dari id_pegawai (PK tabel pegawai).
     * master_jadwal_temp.id_dosen / id_dosen2 merujuk ke pegawai_biodata.id.
     */
    private function getBiodataId(int $idPegawai): ?int
    {
        $row = DB::table('pegawai_biodata')
            ->where('id_pegawai', $idPegawai)
            ->select('id')
            ->first();

        return $row ? (int) $row->id : null;
    }

    /**
     * Hitung nilai huruf dari nilai akhir (angka).
     */
    private function hitungHuruf(float $nilai): string
    {
        if ($nilai >= 85) return 'A';
        if ($nilai >= 80) return 'AB';
        if ($nilai >= 75) return 'B';
        if ($nilai >= 70) return 'BC';
        if ($nilai >= 60) return 'C';
        if ($nilai >= 50) return 'D';
        return 'E';
    }

    /**
     * Halaman daftar jadwal yang diampu dosen yang sedang login.
     */
    public function index()
    {
        $pegawai = Auth::guard('pegawai')->user();
        if (!$pegawai) abort(403);

        $biodataId = $this->getBiodataId((int) $pegawai->id);

        $jadwal = DB::table('master_jadwal_temp as mjt')
            ->select(
                'mjt.*',
                'mmk.nama_mata_kuliah',
                'mmk.jumlah_sks',
                'mta.awal',
                'mta.akhir',
                'mta.jenis',
                DB::raw("COUNT(DISTINCT mk.id) as jumlah_mahasiswa"),
                DB::raw("COUNT(DISTINCT mp.id) as jumlah_pertemuan")
            )
            ->leftJoin('master_mata_kuliah as mmk', 'mmk.kode_mata_kuliah', '=', 'mjt.kode_mata_kuliah')
            ->leftJoin('master_tahun_ajaran as mta', 'mta.id', '=', 'mjt.id_tahun')
            ->leftJoin('master_krs_temp as mk', function ($join) {
                $join->on('mk.id_jadwal', '=', 'mjt.id')
                     ->on('mk.id_tahun', '=', 'mjt.id_tahun');
            })
            ->leftJoin('master_pertemuan as mp', 'mp.id_jadwal', '=', 'mjt.id')
            ->where(function ($q) use ($biodataId) {
                $q->where('mjt.id_dosen', $biodataId)
                  ->orWhere('mjt.id_dosen2', $biodataId);
            })
            ->groupBy(
                'mjt.id', 'mjt.kode_jadwal', 'mjt.id_dosen', 'mjt.id_dosen2', 'mjt.id_tahun',
                'mjt.kode_mata_kuliah', 'mjt.hari', 'mjt.sesi', 'mjt.ruang', 'mjt.kelas',
                'mjt.rombel', 'mjt.kuota_diambil', 'mjt.status', 'mjt.tipe_mhs', 'mjt.rps', 'mjt.kp',
                'mmk.nama_mata_kuliah', 'mmk.jumlah_sks',
                'mta.awal', 'mta.akhir', 'mta.jenis'
            )
            ->orderBy('mta.awal', 'desc')
            ->get();

        // Ambil distribusi huruf per jadwal
        $jadwalIds = $jadwal->pluck('id')->toArray();
        $distribusi = [];
        if (!empty($jadwalIds)) {
            $nilaiRows = DB::table('master_nilai')
                ->whereIn('id_jadwal', $jadwalIds)
                ->whereNotNull('nhuruf')
                ->select('id_jadwal', 'nhuruf', DB::raw('COUNT(*) as jml'))
                ->groupBy('id_jadwal', 'nhuruf')
                ->get();

            foreach ($nilaiRows as $n) {
                $distribusi[$n->id_jadwal][$n->nhuruf] = (int) $n->jml;
            }
        }

        return view('pegawai.nilai.index', [
            'title'       => 'Daftar Matakuliah',
            'CurrentPage' => 'content',
            'jadwal'      => $jadwal,
            'distribusi'  => $distribusi,
            'no'          => 1,
        ]);
    }

    /**
     * Form input nilai mahasiswa per jadwal.
     */
    public function input(string $id_jadwal)
    {
        $pegawai = Auth::guard('pegawai')->user();
        if (!$pegawai) abort(403);

        $biodataId = $this->getBiodataId((int) $pegawai->id);

        $jadwal = DB::table('master_jadwal_temp as mjt')
            ->select(
                'mjt.*',
                'mmk.nama_mata_kuliah',
                'mmk.jumlah_sks',
                'mta.awal',
                'mta.akhir',
                'mta.jenis',
                DB::raw("CONCAT(COALESCE(pb.gelar_depan,''), ' ', COALESCE(pb.nama_lengkap,''), ' ', COALESCE(pb.gelar_belakang,'')) as nama_dosen")
            )
            ->leftJoin('master_mata_kuliah as mmk', 'mmk.kode_mata_kuliah', '=', 'mjt.kode_mata_kuliah')
            ->leftJoin('master_tahun_ajaran as mta', 'mta.id', '=', 'mjt.id_tahun')
            ->leftJoin('pegawai_biodata as pb', 'pb.id', '=', 'mjt.id_dosen')
            ->where('mjt.id', (int) $id_jadwal)
            ->first();

        if (!$jadwal) {
            return redirect('dosen/ujian')->with('error', 'Jadwal tidak ditemukan.');
        }

        // Validasi kepemilikan: hanya dosen yang mengampu yang boleh akses
        if ($jadwal->id_dosen != $biodataId && $jadwal->id_dosen2 != $biodataId) {
            return redirect('dosen/ujian')->with('error', 'Akses ditolak. Jadwal ini bukan milik Anda.');
        }

        $persentase = DB::table('master_persentase_nilai')
            ->where('id_jadwal', (int) $id_jadwal)
            ->first();

        $mahasiswa = DB::table('master_krs_temp as mk')
            ->select(
                'mk.nim',
                'mk.id_jadwal',
                'mk.id_tahun',
                'mhs.nama',
                'mn.ntugas',
                'mn.nuts',
                'mn.nuas',
                'mn.nakhir',
                'mn.nhuruf'
            )
            ->leftJoin('mahasiswa as mhs', 'mhs.nim', '=', 'mk.nim')
            ->leftJoin('master_nilai as mn', function ($join) {
                $join->on('mn.nim', '=', 'mk.nim')
                     ->on('mn.id_jadwal', '=', 'mk.id_jadwal')
                     ->on('mn.id_tahun', '=', 'mk.id_tahun');
            })
            ->where('mk.id_jadwal', (int) $id_jadwal)
            ->orderBy('mhs.nama')
            ->get();

        $status = DB::table('master_nilai')
            ->where('id_jadwal', (int) $id_jadwal)
            ->where('id_tahun', $jadwal->id_tahun)
            ->selectRaw('COALESCE(MAX(publish_tugas),0) as publish_tugas')
            ->selectRaw('COALESCE(MAX(publish_uts),0) as publish_uts')
            ->selectRaw('COALESCE(MAX(publish_uas),0) as publish_uas')
            ->selectRaw('COALESCE(MAX(validasi_tugas),0) as validasi_tugas')
            ->selectRaw('COALESCE(MAX(validasi_uts),0) as validasi_uts')
            ->selectRaw('COALESCE(MAX(validasi_uas),0) as validasi_uas')
            ->first();

        return view('pegawai.nilai.input', [
            'title'         => 'Input Nilai Mahasiswa',
            'CurrentPage'   => 'content',
            'jadwal'        => $jadwal,
            'mahasiswa'     => $mahasiswa,
            'persentase'    => $persentase,
            'publishStatus' => [
                'tugas' => (int) ($status->publish_tugas ?? 0),
                'uts'   => (int) ($status->publish_uts ?? 0),
                'uas'   => (int) ($status->publish_uas ?? 0),
            ],
            'validasiStatus' => [
                'tugas' => (int) ($status->validasi_tugas ?? 0),
                'uts'   => (int) ($status->validasi_uts ?? 0),
                'uas'   => (int) ($status->validasi_uas ?? 0),
            ],
        ]);
    }

    /**
     * Simpan nilai mahasiswa.
     */
    public function save(Request $request)
    {
        $pegawai = Auth::guard('pegawai')->user();
        if (!$pegawai) abort(403);

        $biodataId = $this->getBiodataId((int) $pegawai->id);

        $id_jadwal = (int) $request->input('id_jadwal');
        $id_tahun  = (int) $request->input('id_tahun');
        $id_dosen  = $biodataId;
        $nims      = $request->input('nim', []);
        $ntugas    = $request->input('ntugas', []);
        $nuts      = $request->input('nuts', []);
        $nuas      = $request->input('nuas', []);

        // Validasi kepemilikan jadwal
        $jadwal = DB::table('master_jadwal_temp')
            ->where('id', $id_jadwal)
            ->where(function ($q) use ($biodataId) {
                $q->where('id_dosen', $biodataId)->orWhere('id_dosen2', $biodataId);
            })
            ->first();

        if (!$jadwal) {
            return redirect('dosen/ujian')->with('error', 'Akses ditolak.');
        }

        $persentase = DB::table('master_persentase_nilai')->where('id_jadwal', $id_jadwal)->first();
        $pct_tugas  = $persentase ? (float) $persentase->ntugas : 30;
        $pct_uts    = $persentase ? (float) $persentase->nuts   : 35;
        $pct_uas    = $persentase ? (float) $persentase->nuas   : 35;

        foreach ($nims as $i => $nim) {
            $nt  = isset($ntugas[$i]) && $ntugas[$i] !== '' ? (float) $ntugas[$i] : null;
            $nu  = isset($nuts[$i])   && $nuts[$i]   !== '' ? (float) $nuts[$i]   : null;
            $nua = isset($nuas[$i])   && $nuas[$i]   !== '' ? (float) $nuas[$i]   : null;

            $nakhir = null;
            $nhuruf = null;
            if ($nt !== null && $nu !== null && $nua !== null) {
                $nakhir = round(($nt * $pct_tugas / 100) + ($nu * $pct_uts / 100) + ($nua * $pct_uas / 100), 2);
                $nhuruf = $this->hitungHuruf($nakhir);
            }

            DB::table('master_nilai')->updateOrInsert(
                ['nim' => $nim, 'id_jadwal' => $id_jadwal, 'id_tahun' => $id_tahun],
                [
                    'ntugas'         => $nt,
                    'nuts'           => $nu,
                    'nuas'           => $nua,
                    'nakhir'         => $nakhir,
                    'nhuruf'         => $nhuruf,
                    'ndosen'         => $id_dosen,
                    'is_krs'         => DB::raw('COALESCE(is_krs, 1)'),
                    'publish_tugas'  => DB::raw('COALESCE(publish_tugas, 0)'),
                    'publish_uts'    => DB::raw('COALESCE(publish_uts, 0)'),
                    'publish_uas'    => DB::raw('COALESCE(publish_uas, 0)'),
                    'validasi_tugas' => DB::raw('COALESCE(validasi_tugas, 0)'),
                    'validasi_uts'   => DB::raw('COALESCE(validasi_uts, 0)'),
                    'validasi_uas'   => DB::raw('COALESCE(validasi_uas, 0)'),
                    'log_date'       => now(),
                ]
            );
        }

        return redirect("dosen/ujian/input/{$id_jadwal}")->with('status', 'Nilai berhasil disimpan!');
    }

    /**
     * Simpan persentase penilaian (Tugas/UTS/UAS).
     */
    public function savePersentase(Request $request)
    {
        $pegawai = Auth::guard('pegawai')->user();
        if (!$pegawai) abort(403);

        $request->validate([
            'id_jadwal' => 'required|integer',
            'pct_tugas' => 'required|numeric|min:0|max:100',
            'pct_uts'   => 'required|numeric|min:0|max:100',
            'pct_uas'   => 'required|numeric|min:0|max:100',
        ]);

        $total = (float) $request->pct_tugas + (float) $request->pct_uts + (float) $request->pct_uas;
        if (round($total, 2) !== 100.0) {
            return redirect()->back()->with('error', 'Total persentase harus 100%.');
        }

        DB::table('master_persentase_nilai')->updateOrInsert(
            ['id_jadwal' => (int) $request->id_jadwal],
            [
                'ntugas'   => $request->pct_tugas,
                'nuts'     => $request->pct_uts,
                'nuas'     => $request->pct_uas,
                'datetime' => now(),
            ]
        );

        return redirect()->back()->with('status', 'Persentase nilai berhasil disimpan.');
    }

    /**
     * Toggle status publish nilai per komponen (tugas/uts/uas).
     */
    public function togglePublish(Request $request)
    {
        $pegawai = Auth::guard('pegawai')->user();
        if (!$pegawai) abort(403);

        $request->validate([
            'id_jadwal' => 'required|integer',
            'id_tahun'  => 'required|integer',
            'component' => 'required|in:tugas,uts,uas',
        ]);

        $biodataId = $this->getBiodataId((int) $pegawai->id);
        $this->ensureNilaiRows((int) $request->id_jadwal, (int) $request->id_tahun, $biodataId);

        $fieldMap = ['tugas' => 'publish_tugas', 'uts' => 'publish_uts', 'uas' => 'publish_uas'];
        $field    = $fieldMap[$request->component];

        $curr = (int) DB::table('master_nilai')
            ->where('id_jadwal', $request->id_jadwal)
            ->where('id_tahun', $request->id_tahun)
            ->max($field);

        DB::table('master_nilai')
            ->where('id_jadwal', $request->id_jadwal)
            ->where('id_tahun', $request->id_tahun)
            ->update([$field => $curr === 1 ? 0 : 1, 'log_date' => now()]);

        return redirect()->back()->with('status', 'Status publish berhasil diperbarui.');
    }

    /**
     * Toggle status validasi nilai per komponen (tugas/uts/uas).
     */
    public function toggleValidasi(Request $request)
    {
        $pegawai = Auth::guard('pegawai')->user();
        if (!$pegawai) abort(403);

        $request->validate([
            'id_jadwal' => 'required|integer',
            'id_tahun'  => 'required|integer',
            'component' => 'required|in:tugas,uts,uas',
        ]);

        $biodataId = $this->getBiodataId((int) $pegawai->id);
        $this->ensureNilaiRows((int) $request->id_jadwal, (int) $request->id_tahun, $biodataId);

        $fieldMap = ['tugas' => 'validasi_tugas', 'uts' => 'validasi_uts', 'uas' => 'validasi_uas'];
        $field    = $fieldMap[$request->component];

        $curr = (int) DB::table('master_nilai')
            ->where('id_jadwal', $request->id_jadwal)
            ->where('id_tahun', $request->id_tahun)
            ->max($field);

        DB::table('master_nilai')
            ->where('id_jadwal', $request->id_jadwal)
            ->where('id_tahun', $request->id_tahun)
            ->update([$field => $curr === 1 ? 0 : 1, 'log_date' => now()]);

        return redirect()->back()->with('status', 'Status validasi berhasil diperbarui.');
    }

    /**
     * Upload nilai dari file Excel/CSV.
     */
    public function upload(Request $request)
    {
        $pegawai = Auth::guard('pegawai')->user();
        if (!$pegawai) abort(403);

        $request->validate([
            'id_jadwal'  => 'required|integer',
            'id_tahun'   => 'required|integer',
            'nilai_file' => 'required|file|mimes:csv,txt,xlsx,xls',
        ]);

        $biodataId = $this->getBiodataId((int) $pegawai->id);
        $idJadwal  = (int) $request->id_jadwal;
        $idTahun   = (int) $request->id_tahun;

        // Validasi kepemilikan
        $owns = DB::table('master_jadwal_temp')
            ->where('id', $idJadwal)
            ->where(function ($q) use ($biodataId) {
                $q->where('id_dosen', $biodataId)->orWhere('id_dosen2', $biodataId);
            })
            ->exists();

        if (!$owns) {
            return redirect()->back()->with('error', 'Akses ditolak.');
        }

        $this->ensureNilaiRows($idJadwal, $idTahun, $biodataId);

        $rows = $this->parseUploadedNilaiFile(
            $request->file('nilai_file')->getRealPath(),
            $request->file('nilai_file')->getClientOriginalExtension()
        );

        if (count($rows) === 0) {
            return redirect()->back()->with('error', 'File kosong atau format kolom tidak sesuai.');
        }

        $persentase = DB::table('master_persentase_nilai')->where('id_jadwal', $idJadwal)->first();
        $pctTugas   = $persentase ? (float) $persentase->ntugas : 30;
        $pctUts     = $persentase ? (float) $persentase->nuts   : 35;
        $pctUas     = $persentase ? (float) $persentase->nuas   : 35;

        $imported = 0;
        $skipped  = 0;

        foreach ($rows as $row) {
            $nim = trim((string) ($row['nim'] ?? ''));
            if ($nim === '') { $skipped++; continue; }

            $exists = DB::table('master_krs_temp')
                ->where('id_jadwal', $idJadwal)
                ->where('id_tahun', $idTahun)
                ->where('nim', $nim)
                ->exists();

            if (!$exists) { $skipped++; continue; }

            $nt = $row['ntugas'];
            $nu = $row['nuts'];
            $na = $row['nuas'];

            $nakhir = null;
            $nhuruf = null;
            if ($nt !== null && $nu !== null && $na !== null) {
                $nakhir = round(($nt * $pctTugas / 100) + ($nu * $pctUts / 100) + ($na * $pctUas / 100), 2);
                $nhuruf = $this->hitungHuruf($nakhir);
            }

            DB::table('master_nilai')->updateOrInsert(
                ['nim' => $nim, 'id_jadwal' => $idJadwal, 'id_tahun' => $idTahun],
                [
                    'ntugas'         => $nt,
                    'nuts'           => $nu,
                    'nuas'           => $na,
                    'nakhir'         => $nakhir,
                    'nhuruf'         => $nhuruf,
                    'ndosen'         => $biodataId,
                    'is_krs'         => DB::raw('COALESCE(is_krs, 1)'),
                    'publish_tugas'  => DB::raw('COALESCE(publish_tugas, 0)'),
                    'publish_uts'    => DB::raw('COALESCE(publish_uts, 0)'),
                    'publish_uas'    => DB::raw('COALESCE(publish_uas, 0)'),
                    'validasi_tugas' => DB::raw('COALESCE(validasi_tugas, 0)'),
                    'validasi_uts'   => DB::raw('COALESCE(validasi_uts, 0)'),
                    'validasi_uas'   => DB::raw('COALESCE(validasi_uas, 0)'),
                    'log_date'       => now(),
                ]
            );

            $imported++;
        }

        return redirect()->back()->with('status', "Upload selesai: {$imported} baris diproses, {$skipped} baris dilewati.");
    }

    /**
     * Download template CSV untuk upload nilai.
     */
    public function downloadTemplate()
    {
        $content  = "nim,ntugas,nuts,nuas\n";
        $content .= "A1242013,80,78,85\n";
        $content .= "A1242014,75,70,80\n";

        return response($content, 200, [
            'Content-Type'        => 'text/csv',
            'Content-Disposition' => 'attachment; filename="template_input_nilai.csv"',
        ]);
    }

    /**
     * Pastikan semua mahasiswa di KRS sudah punya baris di master_nilai.
     */
    private function ensureNilaiRows(int $idJadwal, int $idTahun, $idDosen): void
    {
        $krsRows = DB::table('master_krs_temp')
            ->where('id_jadwal', $idJadwal)
            ->where('id_tahun', $idTahun)
            ->select('nim')
            ->get();

        foreach ($krsRows as $krs) {
            DB::table('master_nilai')->updateOrInsert(
                ['nim' => $krs->nim, 'id_jadwal' => $idJadwal, 'id_tahun' => $idTahun],
                [
                    'ndosen'         => $idDosen,
                    'is_krs'         => DB::raw('COALESCE(is_krs, 1)'),
                    'publish_tugas'  => DB::raw('COALESCE(publish_tugas, 0)'),
                    'publish_uts'    => DB::raw('COALESCE(publish_uts, 0)'),
                    'publish_uas'    => DB::raw('COALESCE(publish_uas, 0)'),
                    'validasi_tugas' => DB::raw('COALESCE(validasi_tugas, 0)'),
                    'validasi_uts'   => DB::raw('COALESCE(validasi_uts, 0)'),
                    'validasi_uas'   => DB::raw('COALESCE(validasi_uas, 0)'),
                    'log_date'       => now(),
                ]
            );
        }
    }

    private function parseUploadedNilaiFile(string $path, string $ext): array
    {
        $ext  = strtolower($ext);
        $rows = [];

        if ($ext === 'csv' || $ext === 'txt') {
            $handle = fopen($path, 'r');
            if (!$handle) return [];

            $header = fgetcsv($handle);
            if (!$header) { fclose($handle); return []; }

            $map = $this->mapHeader($header);
            while (($data = fgetcsv($handle)) !== false) {
                $rows[] = $this->extractRow($data, $map);
            }
            fclose($handle);
            return $rows;
        }

        $spreadsheet = IOFactory::load($path);
        $sheet       = $spreadsheet->getActiveSheet();
        $arr         = $sheet->toArray(null, true, true, false);

        if (count($arr) < 2) return [];

        $map = $this->mapHeader($arr[0]);
        for ($i = 1; $i < count($arr); $i++) {
            $rows[] = $this->extractRow($arr[$i], $map);
        }
        return $rows;
    }

    private function mapHeader(array $header): array
    {
        $map = [];
        foreach ($header as $idx => $col) {
            $col = strtolower(trim((string) $col));
            if (in_array($col, ['nim', 'ntugas', 'nuts', 'nuas'])) {
                $map[$col] = $idx;
            }
        }
        return $map;
    }

    private function extractRow(array $data, array $map): array
    {
        $get = fn(string $key) => isset($map[$key]) && isset($data[$map[$key]]) && $data[$map[$key]] !== ''
            ? (float) $data[$map[$key]]
            : null;

        return [
            'nim'    => isset($map['nim']) ? trim((string) ($data[$map['nim']] ?? '')) : '',
            'ntugas' => $get('ntugas'),
            'nuts'   => $get('nuts'),
            'nuas'   => $get('nuas'),
        ];
    }
}
