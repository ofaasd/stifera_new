                    ...
                    <div class="filter cm-content-box box-primary">
                        ...
                        <div class="cm-content-body form excerpt">
                            ...
                            <div class="card-body pb-4">
                                <form action="{{ url('simpeg/absensi/jam_kerja_master/' . $master->id . '/detail') }}" method="POST">
                                    @csrf
                                    <div class="row">
                                            ...
                                            <tbody>
                                                @foreach(['senin' => 'Senin', 'selasa' => 'Selasa', 'rabu' => 'Rabu', 'kamis' => 'Kamis', 'jumat' => 'Jumat', 'sabtu' => 'Sabtu', 'minggu' => 'Minggu'] as $key => $label)
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <input type="time" class="form-control" name="jam_{{ $key }}_mulai" value="{{ old('jam_' . $key . '_mulai') }}" {{ $key === 'minggu' ? 'required' : '' }}>
                                                        </td>
                                                        <td>
                                                            <input type="time" class="form-control" name="jam_{{ $key }}_selesai" value="{{ old('jam_' . $key . '_selesai') }}" {{ $key === 'minggu' ? 'required' : '' }}>
                                                        </td>
                                                    </tr>
                                                @endforeach
                                            </tbody>
