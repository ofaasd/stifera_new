                        ...
                        <ul class="navbar-nav header-right">
							...
							@php
								if (Auth::guard('mahasiswa')->check()) {
									$hdrUser    = Auth::guard('mahasiswa')->user();
									$hdrName    = $hdrUser->nama ?? $hdrUser->nim ?? 'Mahasiswa';
									$hdrRole    = 'Mahasiswa';
									$hdrFotoFile = (string) ($hdrUser->foto_mhs ?? '');
									$hdrFoto    = ($hdrFotoFile !== '' && file_exists(public_path('assets/foto_mahasiswa/' . $hdrFotoFile)))
										? asset('assets/foto_mahasiswa/' . $hdrFotoFile)
										: null;
									$hdrProfileUrl = route('mahasiswa.profile.edit');
									$logoutRoute   = route('mahasiswa.logout');
								} elseif (Auth::guard('pegawai')->check()) {
									$hdrUser    = Auth::guard('pegawai')->user();
									$hdrBiodata = DB::table('pegawai_biodata')->select('nama_lengkap', 'foto')->where('id_pegawai', (int) $hdrUser->id)->first();
									$hdrName    = $hdrBiodata->nama_lengkap ?? $hdrUser->nama ?? 'Pegawai';
									$hdrRole    = 'Pegawai/Dosen';
									$hdrFotoFile = (string) ($hdrBiodata->foto ?? '');
									$hdrFoto    = ($hdrFotoFile !== '' && $hdrFotoFile !== 'logo.png' && file_exists(public_path('assets/foto_pegawai/' . $hdrFotoFile)))
										? asset('assets/foto_pegawai/' . $hdrFotoFile)
										: null;
									$hdrProfileUrl = route('pegawai.biodata.edit');
									$logoutRoute   = route('pegawai.logout');
								} else {
									$hdrAdminUser = Auth::guard('admin')->user() ?? Auth::user();
									$hdrName    = $hdrAdminUser->name ?? 'Admin';
									$hdrRole    = 'Admin';
									$hdrFoto    = null;
									$hdrProfileUrl = url('dashboard');
									$logoutRoute   = route('logout');
								}
								$hdrInitial = strtoupper(substr((string) $hdrName, 0, 1));
							@endphp
									...
									<a class="nav-link p-0" href="javascript:void(0);" role="button" data-bs-toggle="dropdown" aria-expanded="false">
										<div class="header-info2 d-flex align-items-center">
											<div class="header-media">
												@if($hdrFoto)
													<img src="{{ $hdrFoto }}" alt="{{ $hdrName }}" style="width:45px;height:45px;object-fit:cover;border-radius:50%;">
												@else
													<div class="d-flex align-items-center justify-content-center rounded-circle bg-primary text-white fw-bold" style="width:45px;height:45px;font-size:18px;">{{ $hdrInitial }}</div>
												@endif
											</div>
										</div>
									</a>
									<div class="dropdown-menu dropdown-menu-end">
										<div class="card border-0 mb-0">
											<div class="card-header py-2">
												<div class="products">
													@if($hdrFoto)
														<img src="{{ $hdrFoto }}" class="avatar avatar-md" alt="{{ $hdrName }}" style="object-fit:cover;border-radius:50%;">
													@else
														<div class="d-flex align-items-center justify-content-center rounded-circle bg-primary text-white fw-bold avatar avatar-md" style="font-size:18px;">{{ $hdrInitial }}</div>
													@endif
													<div>
														<h6 class="mb-0">{{ $hdrName }}</h6>
														<span>{{ $hdrRole }}</span>
													</div>
												</div>
											</div>
											<div class="card-body px-0 py-2">
												<a href="{{ $hdrProfileUrl }}" class="dropdown-item ai-icon">
											...
											<div class="card-footer px-0 py-2">
												<form id="header-logout-form" action="{{ $logoutRoute }}" method="POST" class="d-none">
													@csrf
												</form>
												<a href="{{ $logoutRoute }}" onclick="event.preventDefault(); document.getElementById('header-logout-form').submit();" class="dropdown-item ai-icon text-danger">
